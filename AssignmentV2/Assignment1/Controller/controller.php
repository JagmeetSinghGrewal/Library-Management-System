...
class Controller{
    ...
    public function execute(){
        ...
        {     
            ...
            //Strip any html tags
            $strippedName = strip_tags($_POST['name']);           

            //registration form inputs to model to handle database interaction
            //Name is validated by preforming regex 
            if(filter_var($strippedName, FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>"/^(([a-zA-Z\s])*)+$/")))!=FALSE&&!empty($_POST["uname"])&&!empty($_POST["age"])&&!empty($_POST["address"])&&!empty($_POST["password"])&&!empty($_POST["email"])){
