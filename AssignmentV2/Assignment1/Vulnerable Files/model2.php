...
class Model2{
    ...
    public function registerMember($name,$username, $age, $address, $password, $email){
        ...
        //select the books197090 database
        mysqli_select_db($mysqli,"books197090");
    ...
    public function searchDB($author, $pub, $name, $genre){
        ...
        //run the query
        $query = "SELECT books.name as name, books.author as author, books.publisher as publisher, books.genre as genre, borrow.availability as borrow FROM books INNER JOIN borrow ON books.name = borrow.book WHERE author='$author' OR name= '$name' OR publisher='$pub' OR genre='$genre'";
        $result = $mysqli->query($query);
